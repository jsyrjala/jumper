...
pub const CollisionRect = struct {
    ...
    pub fn collidesRect(self: CollisionRect, rect: CollisionRect) bool {
        return (self.position.x < rect.position.x + rect.size.x 
                and self.position.x + self.size.x > rect.position.x 
                and self.position.y < rect.position.y + rect.size.y 
                and self.position.y + self.size.y > rect.position.y);
    }
    ...
    pub fn collidesRay(target: CollisionRect, ray_origin: Vec2, ray_dir: Vec2) ?Collision {
        ...
        if (!math.isNormal(t_far.y) or !math.isNormal(t_far.x)) return null;
        if (!math.isNormal(t_near.y) or !math.isNormal(t_near.x)) return null;
    ...
    pub fn collidesDynamicRect(dynamic: CollisionRect, static: CollisionRect) ?Collision {
        ...
        const collision = expanded_target.collidesRay(
            dynamic.position.add(dynamic.size.scale(0.5)),
            dynamic.velocity.subtract(static.velocity)
        );
